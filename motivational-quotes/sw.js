const CACHE="motivational-quotes-v2";
...
self.addEventListener("activate",event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&k.startsWith("motivational-quotes-")).map(k=>caches.delete(k)))).then(()=>self.clients.claim()))});
